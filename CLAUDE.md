...
npm run check:exports  # decrypt + verify wallet export files under DATA_PATH/exports
...
- **Reusable flow primitives** live in `src/bot/patterns/` and are wired once at startup via `initPatterns(bot, sessions)`: `confirmFlow(ctx, …)` (yes/no confirmation with registered global actions), `inputPrompt(ctx, …)` (await a one-shot text reply), and `createPaginator(bot, …)` (paged lists). Prefer these over hand-rolling per-feature confirm/prompt/pagination state.
- **Keyboards, UI text, callbacks** are centralized: `src/bot/keyboards/`, `src/bot/i18n/` (the locale bundle `fr.js`, the `t()`/`MESSAGES`/`EMOJIS` helpers in `index.js`, plus presentation helpers `formatters.js` / `wallet-display.js`), and `src/bot/constants/callbacks.js`. Reuse these rather than inlining strings or callback data.
...
- **Money amounts**: a UI amount is **never** turned into on-chain base units with float math (`Number(x) * 10**decimals`, `parseFloat`, `Math.round`) — those lose precision and `Math.round` can round *up* past what the user typed. Every provider converts via `uiToBaseUnits(amount, decimals)` (`src/shared/amounts.js`): a pure string→`BigInt` parse that truncates excess decimals (rounds **down**, never over-sends) and handles scientific notation. Each `sendTransaction` path then rejects a result of `0n` ("Montant inférieur au minimum transférable") so dust below 1 base unit (1 sat / lamport / wei…) can't silently become a 0-value tx. Decimals come from the chain/token config (e.g. EVM reads `tokenContract.decimals()` live, so BNB-chain USDT's 18 decimals are handled automatically), not a hardcoded table. Tested in `tests/amounts.test.js`.
