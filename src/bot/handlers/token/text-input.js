...
export function setupTokenTextInput(bot, storage, walletService, sessions) {
  ...
  logger.info('[TOKEN_TEXT_INPUT] Loaded');
}
...
async function handleTokenSupply(ctx, text, storage, sessions) {
  ...
  if (supply > 100000000000) {
    return ctx.reply('❌ Supply trop élevée.\n\nMaximum: 100 milliards (100000000000)', {
      parse_mode: 'Markdown',
    });
  }
...
async function handleTokenConfirm(ctx, storage, sessions) {
  ...
  logger.info('[TOKEN_CONFIRM] Starting...', {
    chatId,
    decimals: data.decimals,
    supply: data.supply,
  ...
  if (!data.walletPrivateKey || !data.decimals || !data.supply) {
    logger.error('[TOKEN_CONFIRM] Missing data', {
      chatId,
      decimals: data.decimals,
      supply: data.supply,
    });
    return ctx.editMessageText('❌ Donnees incompletes. Veuillez recommencer le processus /mint.', {
      parse_mode: 'Markdown',
      ...mainMenuKeyboard(),
    });
  }
  ...
  try {
    logger.info('[TOKEN_CONFIRM] Creating mint', { chatId, decimals: data.decimals });

    // Step 1: Create Mint
    const mintResult = await TokenService.createMint(data.walletPrivateKey, data.decimals);

    logger.info('[TOKEN_CONFIRM] Mint result', { chatId, success: mintResult.success });
    ...
    if (data.revokeMintAuthority) {
      ...
      if (!revokeResult.success) {
        logger.warn('[TOKEN] Failed to revoke mint authority', { chatId, error: revokeResult.error });
      } else {
    ...
    logger.logError(error, { context: 'handleTokenConfirm', chatId });
    ...
    if (errorMessage.includes('insufficient funds') || errorMessage.includes('Not enough SOL')) {
      errorMessage =
        'Solde SOL insuffisant pour payer les frais de création.\n\n' +
