  ...
  bot.on('text', async (ctx, next) => {
    ...
    try {
      const loadingMsg = await ctx.reply(
        `${EMOJIS.loading} Importation de ton wallet ${chain.toUpperCase()}...`
      );
      ...
      return ctx.reply(
        `❌ Erreur d'importation : ${error.message}\n\nVérifie ta ${type === 'key' ? 'clé privée' : 'phrase de récupération'} et réessaie.`,
        mainMenuKeyboard()
      );
    }
  });
