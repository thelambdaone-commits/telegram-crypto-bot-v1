...
/**
 * Our wallet chain key → the network label Trocador uses for that chain. This is
 * the SINGLE source of truth for network labels — the native coin AND every
 * token on that chain are quoted under this label. If a pair ever returns
 * "indisponible", confirm the label against GET /api/coins — fixing it here
 * fixes the native and all tokens at once (quote-only, so no funds at risk).
 */
...
const EXTRA_COINS = {
  ...
  usdc_ton: { ticker: 'usdc', network: 'TON', symbol: 'USDC', emoji: '💎', name: 'USDC · TON', walletChain: 'ton' },
  dai_ton: { ticker: 'dai', network: 'TON', symbol: 'DAI', emoji: '💎', name: 'DAI · TON', walletChain: 'ton' },
};
...
function buildCoins() {
  const coins = {};
  for (const [chain, network] of Object.entries(CHAIN_NETWORK)) {
